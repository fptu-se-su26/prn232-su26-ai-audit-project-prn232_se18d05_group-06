...
const SlotBooking: React.FC<SlotBookingProps> = ({ isTab = false, onBack }) => {
  const navigate = useNavigate();
  const [scrollY, setScrollY] = useState(0);
  const [bookingType, setBookingType] = useState<string>('INBOUND');
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post<BookingResponseDto>(`${API_BASE_URL}/bookings/create`, {
        ...
        bookingType: bookingType, // Pass selected service type
      });
  ...
  return (
    <div className={isTab ? "text-slate-900 font-sans w-full text-left" : "bg-white min-h-screen flex flex-col font-sans text-on-surface light-surface"}>
      {!isTab && <Header scrollY={scrollY} />}

      <main className={isTab ? "flex-1 pb-16 w-full text-left" : "flex-1 pt-28 pb-16 px-4 max-w-7xl mx-auto w-full text-left"}>
        {/* Banner Title */}
        <div className="mb-10 text-center md:text-left">
          <button 
            onClick={isTab ? onBack : () => navigate('/')}
          ...
          >
            ...
            {isTab ? 'Quay lại Bảng điều khiển' : 'Quay lại trang chủ'}
          </button>
          <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight flex items-center justify-center md:justify-start gap-3">
            <WarehouseIcon className="text-blue-600" size={36} />
            Đặt Lịch Cửa Kho
          </h1>
          <p className={isTab ? "text-slate-600 font-medium mt-2 max-w-2xl text-left" : "text-gray-500 font-medium mt-2 max-w-2xl text-left"}>
        ...
        {/* Form Selection Card */}
        <div className={isTab ? "glass-panel rounded-3xl p-6 mb-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-end text-slate-900 border border-slate-300 text-left" : "bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm mb-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-end text-left"}>
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Chọn Kho Hàng</label>
            <div className="relative">
              <select 
                value={selectedWarehouseId}
                onChange={(e) => setSelectedWarehouseId(Number(e.target.value))}
                disabled={loadingWarehouses}
                className={isTab ? "w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 outline-none transition-all disabled:opacity-50 appearance-none" : "w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-gray-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 outline-none transition-all disabled:opacity-50 appearance-none"}
              >
                {loadingWarehouses ? (
          ...
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Chọn Ngày Đến</label>
            <div className="relative">
              <input 
                type="date"
                value={selectedDate}
                ...
                className={isTab ? "w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 outline-none transition-all" : "w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-gray-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 outline-none transition-all"}
              />
            </div>
          </div>

          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Loại Dịch Vụ</label>
            <div className="relative">
              <select 
                value={bookingType}
        ...
        {/* Visual Slots Grid */}
        <div className={isTab ? "glass-panel rounded-3xl overflow-hidden border border-slate-300 text-left" : "bg-white border border-gray-100 rounded-3xl shadow-2xl shadow-slate-100/50 overflow-hidden"}>
          <div className={isTab ? "p-6 border-b border-slate-200 bg-slate-50/80 flex justify-between items-center flex-wrap gap-4 text-left" : "p-6 border-b border-gray-100 bg-slate-50/50 flex justify-between items-center flex-wrap gap-4 text-left"}>
            <h3 className={isTab ? "font-bold text-slate-800 text-lg flex items-center gap-2" : "font-bold text-gray-800 text-lg flex items-center gap-2"}>
              <Calendar size={18} className="text-blue-600" />
              Tình trạng đặt chỗ ngày {new Date(selectedDate).toLocaleDateString('vi-VN')}
            </h3>
            {/* Legend */}
            <div className={isTab ? "flex gap-4 text-xs font-semibold text-slate-600" : "flex gap-4 text-xs font-semibold"}>
              <div className="flex items-center gap-1.5">
                <span className="w-3.5 h-3.5 bg-green-500 border border-green-600 rounded"></span>
          ...
          {loadingSlots ? (
            <div className="p-20 text-center flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-slate-500 font-bold text-sm">Đang tải lịch trống từ hệ thống...</p>
            </div>
          ) : dockSlots.length === 0 ? (
            <div className="p-20 text-center">
              <AlertTriangle className="text-amber-500 mx-auto mb-4" size={40} />
              <p className={isTab ? "text-slate-800 font-bold" : "text-gray-500 font-bold"}>Không tìm thấy cửa kho bốc dỡ (Dock) nào khả dụng.</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px] border-collapse">
                <thead>
                  <tr className={isTab ? "bg-slate-50 text-slate-600 text-xs font-bold uppercase tracking-wider border-b border-slate-200 text-left" : "bg-slate-50 text-slate-500 text-xs font-bold uppercase tracking-wider border-b border-gray-100 text-left"}>
                    <th className="p-5 text-left w-1/4">Khung Giờ</th>
                    {dockSlots.map((dock) => (
                      <th key={dock.dockName} className={isTab ? "p-5 text-center font-black text-slate-700" : "p-5 text-center font-black text-gray-800"}>
                        {dock.dockName}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {timeRanges.map((timeRow) => (
                    <tr key={timeRow.range} className={isTab ? "border-b border-slate-200/60 hover:bg-slate-50/50 transition-colors text-left" : "border-b border-gray-100/50 hover:bg-slate-50/20 transition-colors text-left"}>
                      <td className={isTab ? "p-5 font-bold text-slate-700 text-sm flex items-center gap-2" : "p-5 font-bold text-gray-700 text-sm flex items-center gap-2"}>
                          ...
                          <td key={dock.dockName} className="p-4 text-center">
                            {slot.isAvailable ? (
                              <button
                                ...
                                className="w-full max-w-[150px] py-3 px-4 bg-green-50 hover:bg-green-100 border border-green-200 text-green-700 rounded-xl text-xs font-bold transition-all hover:scale-[1.02] shadow-sm hover:shadow active:scale-95 border-none outline-none cursor-pointer"
                              >
                                Còn Trống
                              </button>
                            ) : (
                              <div className="w-full max-w-[150px] mx-auto py-3 px-4 bg-red-50/60 border border-red-100 text-red-700 rounded-xl text-xs font-bold flex flex-col items-center justify-center">
                                ...
                                {slot.bookedLicensePlate && (
                                  <span className="text-[10px] mt-0.5 opacity-80 bg-red-100 px-1.5 py-0.5 rounded font-mono text-red-700">
                                    {slot.bookedLicensePlate}
                                  </span>
                                )}
                              </div>
        ...
        {showBookingModal && selectedSlot && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
            <div className="w-full max-w-lg bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-2xl p-6 relative text-slate-800">
              <h3 className="font-bold text-gray-900 text-xl flex items-center gap-2 mb-6 text-left">
              ...
              <div className="bg-blue-50/70 border border-blue-100 rounded-2xl p-4 space-y-2 mb-6 text-sm text-blue-900">
                ...
                <div className="flex justify-between">
                  <span className="font-semibold text-blue-700">Loại dịch vụ:</span>
              ...
              <form onSubmit={handleBookingSubmit} className="space-y-5 text-left">
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 text-left">Biển Số Xe</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 text-left">Email Tài Xế</label>
                  <div className="relative">
        ...
        {successBooking && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
            <div className="w-full max-w-xl bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-2xl p-6 md:p-8 text-center relative text-slate-800">
              ...
              <div className="bg-slate-50 border border-slate-200/60 rounded-2xl p-5 text-left mb-6 space-y-3">
                ...
                {successBooking.bookingType && (
                  <div className="flex justify-between border-b border-slate-200/40 pb-2">
                    <span className="text-xs font-semibold text-gray-400 uppercase">Loại dịch vụ</span>
                    <span className="text-sm font-bold text-gray-800">{successBooking.bookingType}</span>
                  </div>
                )}
                <div className="flex justify-between">
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl text-sm transition-all active:scale-[0.98] shadow-md shadow-blue-500/20 border-none outline-none cursor-pointer"
                >
                  Đặt Lộ Trình Mới
                </button>
                <button
                  onClick={isTab ? onBack : () => navigate('/')}
                  className="flex-1 bg-slate-100 hover:bg-slate-200 text-gray-700 font-bold py-3.5 rounded-xl text-sm transition-all active:scale-[0.98] border-none outline-none cursor-pointer"
                >
                  {isTab ? 'Quay lại Bảng điều khiển' : 'Quay Lại Trang Chủ'}
                </button>
              </div>
