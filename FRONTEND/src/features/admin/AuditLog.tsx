...
const auditLogsData = [
  {
    ...
    user: {
      ...
      avatarUrl: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAQOHbD8-mF5zz9Xq1vt00fxaMDfNuUtDSDtesgyJW6noTTdIKvCNrdFMwLg8F-NpVlRwPUm0TRG81odJXhUDFXMd-FrzY9e0aPShn5-nUBGgXWP58-9cRwJR9Fqi_dBYR4OqiQsxK-yNz5fg4syrk9kl5EzD65SOjUU8HY00f9IzCp1sdE_FKyKqSaRBF_0AiTLJ_l3VpEl2u-upnXFWMKMHUPnfoo7LYzj2kH3MZoydf6w8IeRgyDAl1mxf3XvuDL3zvDvDpfThNS',
    },
    action: {
      title: 'Mass Deletion',
  ...
  },
  {
    ...
    action: {
      ...
      badge: 'Unrecognized Device',
      badgeClass: 'bg-tertiary/10 text-tertiary',
      badgeDotClass: 'bg-tertiary',
    },
  ...
  },
  {
    ...
    user: {
      ...
      initials: 'SYS',
      initialsClass: 'bg-secondary/10 text-secondary',
    },
    action: {
    ...
    <div className="bg-background text-on-surface font-body-md text-body-md min-h-screen flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:ml-[280px] w-full h-screen overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-container-padding z-10 relative">
          <div className="max-w-max-width mx-auto space-y-stack-lg">
            {/* Header & Actions */}
            ...
            {/* Advanced Filters */}
            <div className="glass-card rounded-[18px] p-stack-md flex flex-wrap gap-4 items-center">
            ...
            {/* AI Insights Section */}
            <div className="glass-card rounded-[18px] p-stack-md flex items-center gap-4 border-l-4 border-l-tertiary-fixed">
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="font-body-sm text-body-sm text-on-surface">
                    {auditLogsData.map((log) => (
                      <tr 
                        key={log.id} 
                        className={`group hover:bg-surface-variant/30 transition-colors ${log.hasBottomBorder ? 'border-b border-outline-variant/10' : ''}`}
                      >
                        <td className="p-4 pl-6">
                          <div className="flex items-center gap-3">
                            {log.user.avatarUrl ? (
                              <img alt={log.user.name} className="w-8 h-8 rounded-full" src={log.user.avatarUrl} />
                            ) : (
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-medium ${log.user.initialsClass}`}>
                                {log.user.initials}
                              </div>
                            )}
                            <div>
                              <div className="font-medium">{log.user.name}</div>
                              <div className="text-on-surface-variant text-[12px]">{log.user.email}</div>
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                          <div className="flex flex-col gap-1">
                            <span className={`font-medium ${log.action.titleClass}`}>{log.action.title}</span>
                            {log.action.badge && (
                              <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium w-max ${log.action.badgeClass}`}>
                                <span className={`w-1.5 h-1.5 rounded-full mr-1 ${log.action.badgeDotClass}`}></span> {log.action.badge}
                              </span>
                            )}
                          </div>
                        </td>
